...
| `WORKER_LINKEDIN_PAGE_DELAY_MS`, `WORKER_LINKEDIN_DESCRIPTION_COOLDOWN_MS`, `WORKER_LINKEDIN_AVAILABILITY_COOLDOWN_MS` | LinkedIn pacing (ms): delay between collection pages (default `1200`), cooldown between live job-description fetches (default `5000`, claimed one at a time), and cooldown between live availability checks (default `5000`, which only spaces consecutive checks — they may still interleave with other work). |
